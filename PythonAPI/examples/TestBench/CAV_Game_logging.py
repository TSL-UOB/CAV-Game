...
class log():
	def __init__(self, logging_time_increment):
		...
		self.sim_time_array       = []
		self.fps_array            = []
		self.game_score_array     = []
		

		self.logging_time_increment = logging_time_increment


	def append(self,testNo,repeatNo,agents_list,sim_time,fps,game_score):
		...
		if diff >= self.logging_time_increment:
			for agent in agents_list:
				...
				self.game_score_array.append(game_score)
				try:
	...
	def write_file(self, output_folder, file_name):
		...
		with open(file_path, 'w') as log_file: 
			log_file.write('testNo ,repeatNo, agentNo, agentID, agentType, agentTypeNo, x, y, z, yaw, vel_x, vel_y, vel_z, speed, time, sim_time, fps, game_score\n')
			for i in range(len(self.testNo_array)):
				log_file.write('%d, %d, %d, %d, %s, %d, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %3.3f, %s\n' %\
					(self.testNo_array[i],self.repeatNo_array[i], self.agentNo_array[i], self.agentID_array[i], self.agentType_array[i],\
					self.agentTypeNo_array[i], self.x_array[i], self.y_array[i], self.z_array[i], self.yaw_array[i],self.vel_x_array[i],self.vel_y_array[i],self.vel_z_array[i], self.speed_array[i], self.time_array[i], self.sim_time_array[i], self.fps_array[i], self.game_score_array[i]))
...
try:
	...
	# score file path
	score_file = "../../../Unreal/CarlaUE4/Content/Carla/CAV_Game/Scripts/Score.csv"
	game_score = 0
	...
	while True:
		...
		tests_logs.append(0,0,agents_list,t,fps, game_score)
	...
	tests_logs.write_file("tests_logs","LastRunLog.txt")
	
	list_of_files = glob.glob('../../../scenario_runner_0.9.13/srunner/examples/CAV_Game_Scenario_Storage/*.xosc') # * means all if need specific format then *.csv
	latest_file = max(list_of_files, key=os.path.getctime)
	tests_logs.write_file("../../../scenario_runner_0.9.13/srunner/examples/CAV_Game_Scenario_Storage",latest_file[-24:-5]+"_run_logs.txt")